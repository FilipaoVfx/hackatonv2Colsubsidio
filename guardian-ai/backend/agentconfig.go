...
func DefaultConfig() AgentConfig {
	return AgentConfig{
		...
		// `derivar_humano` salió de los objetivos por defecto: el agente ES el
		// asesor y cierra la venta. La derivación sigue disponible en el
		// catálogo del Studio, pero como EXCEPCIÓN que se activa a propósito.
		Sales: SalesConfig{Goals: []string{
			"resolver_dudas", "calificar_cliente", "recomendar_producto", "cerrar_venta",
		}},
		Safety: SafetyConfig{
...
var salesGoalPhrases = map[string]string{
	...
	"cerrar_venta":        "cerrar tú mismo la vinculación: cotizar, confirmar y dejar a la persona asegurada sin derivarla a nadie",
...
func safetyLevelPhrase(level string) string {
	switch level {
	case "alto":
		return "Ante cualquier duda, dilo con claridad y verifica el dato con el sistema antes de afirmarlo."
	case "medio":
